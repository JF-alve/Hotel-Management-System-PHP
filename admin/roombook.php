...
if (isset($_POST['co'])) {
	...
	if ($st == "Conform") {
		...
		if ($f1 == "NO") {
			...
			$psql = "INSERT INTO `payment`(`id`,`title`,`fname`, `lname`,`Phone`, `troom`, `tbed`, `nroom`, `cin`, `cout`, `ttot`,`meal`, `mepr`, `btot`,`fintot`,`noofdays`)
			 VALUES ('$id','$title','$fname','$lname','$Phone','$troom','$bed','$nroom','$cin','$cout','$ttot','$meal','$mepr','$btot','$fintot','$days')";
			...
			if (mysqli_query($con, $psql)) {
				...
				if (mysqli_query($con, $rpsql)) {
					// Query to fetch the email from the database
					$emailQuery = "SELECT Email FROM `roombook` WHERE id = '$id'";
					$emailResult = mysqli_query($con, $emailQuery);

					// Check if the email exists and fetch it
					if ($emailRow = mysqli_fetch_assoc($emailResult)) {
						...
						$paymentUrl = "https://www.bkash.com/en/products-services/payment?booking_id=$booking_id";
						$body = "Dear $title $fname $lname,\n\nYour room booking has been confirmed.\n\nDetails:\n"
							. "Room Type: $troom\n"
							. "Number of Rooms: $nroom\n"
							. "Check-in Date: $cin\n"
							. "Check-out Date: $cout\n"
							. "Total Amount: $$fintot\n"
							. "You can make your payment using the following link:\n$paymentUrl\n\n"
							. "\nThank you for choosing us!";

						$headers = "From: yourhotel@example.com";

						// Send the email
						if (mail($recipientEmail, $subject, $body, $headers)) {
							echo "<script type='text/javascript'> alert('Booking Confirmation Email successfully sent to $recipientEmail');</script>";
						} else {
							echo "<script type='text/javascript'> alert('Email sending failed.');</script>";
						}
					} else {
						echo "<script type='text/javascript'> alert('Failed to retrieve email for this booking.');</script>";
					}
		...
		$urb = "UPDATE `roombook` SET `stat`='Rejected' WHERE id = '$id'";
		...
		if (mysqli_query($con, $urb)) {
			...
			if ($emailRow = mysqli_fetch_assoc($emailResult)) {
				...
				// Send the email
				if (mail($recipientEmail, $subject, $body, $headers)) {
					echo "<script type='text/javascript'> alert('Booking Rejection Email successfully sent to $recipientEmail');</script>";
				} else {
					echo "<script type='text/javascript'> alert('Email sending failed.');</script>";
				}
			} else {
